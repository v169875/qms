...
<!-- 		 <view @click="autoPlay" style="position: fixed; margin-top: 100upx; right: 40upx;">
		...
		<view :style="'width: '+ windowWidth +'px; height: '+ boxStyle.height +'px;'">
			<!-- 
			 ...
			 -->
			<list @loadmore="getData" @scroll="scrolls" :loadmoreoffset="wHeight" :show-scrollbar="false" ref="listBox" :pagingEnabled="true" :scrollable="scrollable">
				<!-- 刷新模块 -->
				...
				<cell v-for="(item,i) in dataList" :key="i">
					...
					<div :style="'width: '+ windowWidth +'px; height: '+ boxStyle.height +'px;'" @disappear="stop()">
						<view v-if="isChange">
							...
							<view class="userInfo">
								<!-- 1.头像 -->
								<image v-if="item.isShowProgressBarTime == false && item.user" @click="tozuozhe" class="userAvatar" :src="item.user.avatar_format" mode="aspectFill"></image>
								<!-- 2.点赞 -->
								<view v-if="item.isShowProgressBarTime == false" @click="cLike(item);" style="opacity: 0.9; margin-top: 17px;">
									<image v-if="item.is_like" src="@/static/video/xin.png" style="width: 40px; height: 40px; position: absolute; right: 6px;"></image>
									<image v-if="!item.is_like" src="@/static/video/xin-2.png" style="width: 40px; height: 40px; position: absolute; right: 6px;"></image>
									<text style="color: #FFFFFF; margin-top: 5px; font-size: 14px; text-align: center; margin-top: 40px; font-weight: bold;" :class="{'likeNumActive':item.is_like}">{{item.like}}</text>
								</view>
								<!-- 3.评论 -->
								<view v-if="item.isShowProgressBarTime == false" class="comment" @click="toComment(i)" style="opacity: 0.9; margin-top: 17px;">
									<image src="@/static/img/index/comment-icon.png" style="width: 35px; height: 35px; position: absolute; right: 7px;"></image>
									<text style="color: #FFFFFF; margin-top: 5px; font-size: 14px; font-weight: bold; text-align: center; margin-top: 40px;">{{item.comment}}</text>
								</view>
								<!-- 收藏 -->
								<view v-if="item.isShowProgressBarTime == false" class="comment" @click="handleFavorite(item)" style="opacity: 0.9; margin-top: 17px;">
									<image src="/static/video/collect-video-icon.png" style="width: 35px; height: 35px; position: absolute; right: 7px;"></image>
									<text style="color: #FFFFFF; margin-top: 5px; font-size: 14px; font-weight: bold; text-align: center; margin-top: 40px;">{{item.favorite}}</text>
								</view>
								<!-- 4.分享 -->
								<view v-if="item.isShowProgressBarTime == false" @click="share" style="opacity: 0.9; margin-top: 17px;">
									...
									<text style="color: #FFFFFF; margin-top: 5px; font-size: 14px; text-align: center; font-weight: bold; margin-top: 40px;">{{item.transmit}}</text>
								</view>
							</view>
							<!-- 最底下的文字部分 -->
							<view class="content" v-if="dataList.length !== 0 && dataList[k].isShowProgressBarTime == false">
								<text class="userName" :style="'width: '+ (windowWidth - 90) +'px;'" v-if="item.user">{{item.user.nickname}}</text><!-- i={{i}} -->
                <text class="words" :style="'width: '+ (windowWidth - 90) +'px;'">{{item.video_title}}</text><!-- k={{k}} -->
						...
						<view v-if="Math.abs(k-i)<=1&&!isChange">
							<view class="root">
								<!-- 
								具体视频参数可以参考官方文档
								说明：
    ...
    data() {
		...
		watch:{
			k(k,old_k){//监听 k 值的变化，可以控制视频的播放与暂停
				...
				uni.createVideoContext(this.dataList[old_k]._id+''+old_k,this).play()
...
1     }
    ...
    methods: {
        ...
        if(res.code == 1){
          ...
          console.log('tytytytyt',this.listQuery.page);
        }
			},
			touchstart(event){
      ...
      async handleFavorite(item){
        ...
        console.log('gggggg',this.dataList)
				if(Number(options.option) == 0){
					this.dataList[0].state = "play";
					setTimeout(()=>{
						//这里的延迟是为了避免执行时间太快而直接跳过执行的 bug
						uni.createVideoContext(this.dataList[0]._id+''+this.k,this).seek(0)
					...
					const dom = uni.requireNativePlugin('dom')
					let doms = 'item' + Number(options.option);
					setTimeout(()=>{
						let el = this.$refs[doms][0]
						dom.scrollToElement(el,{
							offset: 0,
			...
			toComment(index){
				...
				let user = this.dataList[index].user;
				this.$set(user,'id',this.dataList[index].user_id);
