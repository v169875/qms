<template>
	...
	<view class="order-confirm-content">
		...
		<view class="goods-info flex m-l-36 m-r-36 p-b-28" v-for="item in courseDetail">
			<u-image width="154rpx" height="154rpx" border-radius="6" :src="item.classes_cover_image_format"
				mode="aspectFill"></u-image>
			<view class="u-flex-1 m-l-16">
				<view class="hidden_two font-30 goods-title">{{item.classes_name || ''}}</view>
				<view class="flex col-center row-between m-t-28">
					<view class="main-color font-32 text-bold">{{item.classes_current_price}}</view>
					<!-- 拼团订单禁止修改数量 -->
					<u-number-box :min="1" v-model="item.value" @change="valChange" :disabled="is_group"></u-number-box>
				</view>
			</view>
		</view>
		...
		<view class="flex col-center row-between bottom-order-confirm">
			...
			<view class="flex col-center right-btn text-white">
				...
				<view class="m-l-16 blue-btn main-bg-color radius-50" @tap="handleSubmit()">提交报名</view>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			totalPrice: function() {
				let num = 0
				this.courseDetail.forEach(item => {
					num += item.classes_current_price * item.value
				})
		...
		onLoad(options) {
			if (options.classes_id) {
				this.classesid = options.classes_id.split(',')
				this.nums = options.num.split(',')
				this.cart_id = options.cart_id.split(',')
				this.classesid.forEach((item, index) => {
					this.getCourseDetail(item, index);
				})
		...
		methods: {
			...
			// 获取课程详情
			async getCourseDetail(classes_id, index) {
				let data = {
				...
				if (res.code == 1) {
					let data = {
						value: Number(this.nums[index]),
						cart_id: this.cart_id[index],
						...res.data
					}
					this.courseDetail.push(data);
			...
			async handleClassesOrderSubmit() {
				...
				let data = {
					current_school_id: this.courseDetail[0].school_id, //学校id
				...
				if (this.is_group) {
					...
					//普通订单提交
					res = await this.$api.submitOrder(data);
				}
				if (res.code == 1) {
