<template>
	<view>
		...
		<text style="font-size: 24upx; font-weight: bold; text-align: center; color: #FFFFFF;">{{pinlunNum}}条评论</text>
		<scroll-view :style="'width: '+ Width +'px; height: '+ (Height/num) +'px; margin-top: 10px; background-color: #242424; display: flex; flex-direction: column;'" :scroll-y="true">
			...
			<block v-for="(list,index) in pinlunList">
				<view :style="'width: '+ Width +'px; display: flex; flex-direction: row;'">
					<!-- 1.用户的头像 -->
					<image :src="list.avatar_format" mode="aspectFill" style="width: 37.5px; height: 37.5px; border-radius: 25px; margin-top: 20px; margin-left: 15px;"></image>
					<!-- 2.一级评论 -->
					<view :style="'width: '+ (Width*0.8) +'px; display: flex; flex-direction: column; margin-top: 20px; margin-left: 10px;'">
						<!-- 3.用户名称，并自动判断是否为视频作者 -->
						<view style="display: flex; flex-direction: row;">
							<text style="font-size: 12px; font-weight: bold; color: #a3a1a4;">{{list.user.nickname}}</text>
							<!-- 4.如果是视频作者就显示 作者 -->
							<view v-if="videouserID == list.user_id" style="background-color: #E43D33; border-radius: 2.5px; margin-left: 7.5px;">
						...
						-->
						<view v-if="list.length !== 0" @click="huifu(index)" :style="'width: '+ (Width*0.78) +'px; display: flex; flex-direction: row; flex-wrap: wrap; margin-top: 7px;'">
							<block>
								<text :style="'font-size: 15px; font-weight: bold; color: #FFFFFF; margin-left: -3upx;'">{{list.content}}</text>
						...
						<view :style="'width: '+ (Width*0.8) +'px; height: 20px; display: flex; flex-direction: row; margin-top: 12.5px;'">
							<text style="font-size: 12px; color: #a3a1a4;">{{list.createtime_format}}</text>
							...
							<image v-if="!list.is_like" @click="tolike(index)" src="@/static/douyin/dianzan-7.png" style="width: 17.5px; height: 17.5px; position: absolute; right: 22.5px; margin-left: 2.5px;"></image>
							<image v-if="list.is_like" @click="tolike(index)" src="@/static/douyin/dianzan-6.png" style="width: 17.5px; height: 17.5px; position: absolute; right: 22.5px; margin-left: 2.5px;"></image>
							<text @click="tolike(index)" style="font-size: 12px; color: #a3a1a4; position: absolute; right: 45px; margin-top: 5px;">{{list.like}}</text>
						</view>
						<!-- 8.用户回复的子评论会显示在这里 
						...
						-->
						<block v-for="(li,inde) in list.reply_list" v-if="update">
							<view :style="'width: '+ (Width*0.8) +'px; display: flex; flex-direction: row;'">
								<!-- 9.子评论 用户头像 -->
								<image v-if="li.user" :src="li.user.avatar_format" mode="aspectFill" style="width: 22.5px; height: 22.5px; border-radius: 25px; margin-top: 20px;"></image>
								<!-- 10.动态计算宽度 子评论列表 -->
								<view :style="'width: '+ (Width*0.8*0.85) +'px; display: flex;  flex-direction: column; margin-top: 20px; margin-left: 10px;'">
									<view style="display: flex; flex-direction: row;">
										<view style="display: flex; flex-direction: row;">
											<!-- 11.回复评论的人显示在这里 -->
											<text style="font-size: 12px; font-weight: bold; color: #a3a1a4;">{{li.user.nickname}}</text>
											<!-- 12.同时要判断评论人是不是作者，如果是作者就在这里显示 -->
											<view v-if="userID == li.user_id" style="background-color: #E43D33; border-radius: 2.5px; margin-left: 7.5px;">
												<text style="font-size: 8px; font-weight: bold; padding: 2.5px; padding-left: 4px; padding-right: 4px; font-weight: bold; color: #FFFFFF;">作者</text>
											</view>
										</view>
										<!-- 13.图片是一个向右的小图标 -->
										...
										<view style="display: flex; flex-direction: row; margin-left: 5px;">
											<text style="font-size: 12px; font-weight: bold; color: #a3a1a4;">{{li.replyuser.nickname}}</text>
											<!-- 15.同时要判断评论人是不是作者，如果是作者就在这里显示 -->
											<view v-if="userID == li.reply_user_id" style="background-color: #E43D33; border-radius: 2.5px; margin-left: 7.5px;">
												<text style="font-size: 8px; font-weight: bold; padding: 2.5px; padding-left: 4px; padding-right: 4px; font-weight: bold; color: #FFFFFF;">作者</text>
											</view>
										</view>
									</view>
									...
									-->
									<view v-if="li.length !== 0" @click="sonhuifu(index,inde)" :style="'width: '+ (Width*0.8*0.85) +'px; display: flex; flex-direction: row; flex-wrap: wrap; margin-top: 7px;'">
										<block>
											<text :style="'font-size: 15px; font-weight: bold; color: #FFFFFF; margin-left: -3upx;'">{{li.content}}</text>
									...
									<view :style="'width: '+ (Width*0.8*0.85) +'px; height: 20px; display: flex; flex-direction: row; margin-top: 12.5px;'">
										<text style="font-size: 12px; color: #a3a1a4;">{{li.createtime_format}}</text>
										...
										<image v-if="!pinlunList[index].reply_list[inde].is_like" @click="tosonlike(index,inde)" src="@/static/douyin/dianzan-7.png" style="width: 17.5px; height: 17.5px; position: absolute; right: 10px; margin-left: 2.5px;"></image>
										<image v-if="pinlunList[index].reply_list[inde].is_like" @click="tosonlike(index,inde)" src="@/static/douyin/dianzan-6.png" style="width: 17.5px; height: 17.5px; position: absolute; right: 10px; margin-left: 2.5px;"></image>
										<text @click="tosonlike(index,inde)" style="font-size: 12px; color: #a3a1a4; position: absolute; right: 31px; margin-top: 5px;">{{li.like}}</text>
									</view>
								</view>
							</view>
						</block>
						<view v-if="list.isOpenComment && list.reply_list.length > 0" @click="handleMoreComment(index)" :style="'width: '+ (Width*0.8) +'px; display: flex; flex-direction: row; margin-top: 30upx;'">
						  <text style="font-size: 12px; font-weight: bold; color: #cdcbd4; margin-left: 12upx;">点击加载更多评论</text>
						</view>
						<!-- 19。显示 【展开xx条评论】、【收起评论】 -->
			...
			<view @click="handleAllMoreComment()" :style="'width: '+ (Width*1) +'px; display: flex; flex-direction: row; margin-top: 30upx;'" style="justify-content: center;">
			  <text style="font-size: 12px; font-weight: bold; color: #cdcbd4; margin-left: 12upx;">{{allMoreCommentText}}</text>
			</view>
			<view class="" style="height: 70px;" v-if="!isNotIndex">
	...
	export default {
		...
		data() {
			return {
				...
				allMoreCommentText:'点击展开更多评论',
				newPinglunNum:0, //全部的评论数量
				isMore: true,
				// 每一个参数都重要
				// 解释如下：
				// ----- start ----- 评论输入框部分
		...
		methods: {
			...
			*/ 
			async getnewpinlun(){
			  ...
			  console.log('重新调用了111111111',data)
			  const res = await api.getVideoDetailCommentAndReply(data);
			  console.log('重新调用了2222222222',res)
			  if(res.code == 1){
			    if(res.data.data.length > 0){
			      this.pinlunList = [];
			      this.pinlunListX = [];
			      var newPinglunList = [];
			      var pin = res.data.data;
			      var pins = res.data.data;
			      this.listQuery.page++;
			      this.videouserID = res.data.data[0].user_id;
			      this.pinlunNum = pin.length;
			      for(let i=0;i<pin.length;i++){
			      	if(pin[i].reply_count !== 0){
			          this.pinlunNum += pin[i].reply_count;
			          newPinglunList.push(pin[i]);
			          this.pinlun_list = JSON.parse(JSON.stringify(newPinglunList));
			      		pin[i].sonpinlunText = "展开" + pin[i].reply_count + "条回复";
			      		pin[i].sonpinlunLength = pin[i].reply_count;
			          pin[i].isOpenComment = false;
			      		pin[i].reply_list = [];
			      		this.pinlunList.push(pin[i]);
			      	} else {
			...
			async handleAllMoreComment(){
			  ...
			  if(res.code == 1){
			    if(res.data.data.length > 0){
			      ...
			      this.listQuery.page++;
			      this.videouserID = res.data.data[0].user_id;
			      this.newPinglunNum = pin.length;
			      for(let i=0;i<pin.length;i++){
			      	if(pin[i].reply_count !== 0){
			          this.newPinglunNum += pin[i].reply_count;
			          newPinglunList.push(pin[i]);
			          this.pinlun_list = JSON.parse(JSON.stringify(newPinglunList));
			      		pin[i].sonpinlunText = "展开" + pin[i].reply_count + "条回复";
			      		pin[i].sonpinlunLength = pin[i].reply_count;
			      ...
			      this.pinlunNum = this.pinlunNum + this.newPinglunNum;
			    }else{
			      this.allMoreCommentText = '已经展示全部了';
			    }
			  }
			},
			async sendSMS(){
			  ...
			  if(this.value == ''){
			    ...
			    let msgData = {
			      ...
			      reply_user_id:this.gethuifuUserID,
			      content:this.value,
			    }
			    this.isSend = false;
			    // 3. 评论正在发送
			    ...
			    const res = await api.videoCommentReplyStore(msgData);
			    if(res.code == 1){
			      uni.hideLoading();
			      ...
			      this.pinlunNum = 0;
			      this.pinlunListX = [];
			      this.subListQuery.page = 1;
			      this.iszhangkai = false;
			      this.value = "";
			      this.imageURL = "";
			      this.$refs.openPinglun.close();
			      this.isSend = true;
			      console.log('发送之后的查询11111',this.listQuery)
			      this.getnewpinlun();
			    }
					// uni.request({
			    ...
			    // 一级评论
			    const res = await api.videoCommentStore(msgData);
			    if(res.code == 1){
			      uni.hideLoading();
			      this.listQuery.page = 1;
			      this.pinlunList = [];
			      ...
			      this.$refs.openPinglun.close();
			      this.isSend = true;
			      console.log('发送之后的查询2222',this.listQuery)
			      this.getnewpinlun();
			    }
				}
			},
			async tolike(index){
			  ...
			  let data = {
			    type:2,
			    like_id:this.pinlunList[index].id,
			  };
			  let res = {};
			  // 点赞
			  if(this.pinlunList[index].is_like == false){
			    res = await api.addVideoLike(data);
			    if(res.code == 1){
			      this.pinlunList[index].like++;
			      this.pinlunList[index].is_like = true;
			    }
			  }else{
			    res = await api.subVideoLike(data);
			    if(res.code == 1){
			      this.pinlunList[index].like--;
			      this.pinlunList[index].is_like = false;
			    }
			  }
			},
			// 处理子评论
			async tosonlike(index,inde){
			  let data = {
			    type:2,
			    like_id:this.pinlunList[index].reply_list[inde].id,
			  };
			  let res = {};
			  // 点赞
			  if(this.pinlunList[index].reply_list[inde].is_like == false){
			    res = await api.addVideoLike(data);
			    if(res.code == 1){
			      this.pinlunList[index].reply_list[inde].like++;
			      this.pinlunList[index].reply_list[inde].is_like = true;
			    }
			  }else{
			    res = await api.subVideoLike(data);
			    if(res.code == 1){
			      this.pinlunList[index].reply_list[inde].like--;
			      this.pinlunList[index].reply_list[inde].is_like = false;
			    }
			  }
			},
			// 展开第一页评论
			async zhangkai(index){
			  let data = {
			    video_id:this.videoID,
			    comment_id:this.pinlunList[index].id,
			    ...this.subListQuery,
			  }
			  const res = await api.getVideoDetailCommentSubReply(data);
				console.log(res)
				console.log(this.pinlunList[index])
			  if(res.code == 1){
			    // 1.点击展开评论，一开始是不展开的（如果点击展开，就把 评论的副本的子评论赋值给当前页面页面评论，这样当前页面就可以显示子评论了）
			    if(this.pinlunList[index].isOpenComment == false){
			      this.pinlunList[index].reply_list = this.pinlunList[index].reply_list.concat(res.data.data);
			      if(this.pinlunList[index].isOpenComment == false && res.data.data.length > 0){
			        this.pinlunList[index].sonpinlunText = "收起评论";
			        this.pinlunList[index].isOpenComment = true;
			        this.subListQuery.page++;
			        this.update = false;
			        this.update = true;
			      } else{
			        this.subListQuery.page = 1;
							// this.pinlunList[index].isOpenComment = false;
							this.pinlunList[index].reply_list = [];
			        this.pinlunList[index].sonpinlunText = "展开" + this.pinlunList[index].reply_count + "条回复";
			      }
			...
			*/ 
			...
			sonhuifu(index,inde){
				...
				*/
				...
				this.gethuifuUser = this.pinlunList[index].reply_list[inde].user.nickname;
				this.gethuifuUserID = this.pinlunList[index].reply_list[inde].user_id;
				this.comment_id = this.pinlunList[index].id;
				this.istohuifu = true;
			...
			huifu(index){
				...
				（4）打开评论框
				*/
				this.huifuUser = uni.getStorageSync("user").nickname;
				this.gethuifuUser = this.pinlunList[index].user.nickname;
				this.gethuifuUserID = this.pinlunList[index].user.id;
				this.comment_id = this.pinlunList[index].id;
				this.istohuifu = true;
				this.placeholder = "回复：" + this.pinlunList[index].user.nickname;
				this.huifuindex = index;
				this.openPinglun();
			},
			clicknowImage(index){
