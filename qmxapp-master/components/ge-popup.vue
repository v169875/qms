	...
	<view>
		...
		<view v-else-if="type==2">
			<view class="redbox">
				...
				<image @click="gonavigate" src="/static/hongbao_bg.png"></image>
				...
				<view class="wrapBox">
					...
					<br>
          <br>
					<view class="btn" @click="gonavigate">去领更多福利</view>
				</view>
		...
		<view v-else-if="type==4">
			<view class="updateBox">
				<view class="tubiao"><image src="../static/new-version.png"></image></view>
				<view class="wrapbox">
					...
					<view class="btnbox">
						<view v-if="!forceUpdate"  class="btnno" @click="close">暂不升级</view><view class="btnUp" @click="eventClick">立即升级</view>
					</view>
				</view>
			</view>
		</view>
      ...
      forceUpdate:{
		...
		methods:{
			gonavigate(){
			        uni.navigateTo({
			            //保留当前页面，跳转到积分中心
			            url: '/pages/mine/integralCenter/integralCenter'
			        })
			    },
			close(){
				this.$emit('close');
	...
	.redbox{
		...
		.wrapBox{
			...
			.btn{
				...
				padding:20rpx 0px;
				text-align:center;
				background:linear-gradient(to bottom,#FFE06B,#FFE06B);
				border-radius: 34px;
				font-size: 30rpx;
				color:#e06d36;
				font-weight: bold;
